...
export function getFocusLocationToRight(state: State, location: Location | undefined): Location | undefined {
  if (location) {
    if (location.column.idx + 1 > state.cellMatrix.last.column.idx) {
      if (location.row.idx + 1 <= state.cellMatrix.last.row.idx) {
        const nextFocusableLocation = getNextFocusableLocation(state, location.row.idx + 1, 0);
        if (nextFocusableLocation) {
          return nextFocusableLocation;
        }
      }
